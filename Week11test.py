...
def test_predictions(datasetPath, threshold, percentage, prediction_function, worksheet, row, col):
    ...
    preds = []
    for pair in no_edge_pairs_list:
        u = list(nx.common_neighbors(Ga,pair[0], pair[1]))
        value = 0
        #numerator
        for item in u:
            Nu = list(Ga.neighbors(item))
            A = Ga.number_of_edges(item, pair[0])
            B = Ga.number_of_edges(item, pair[1])
            value += (A * B) / len(Nu)
